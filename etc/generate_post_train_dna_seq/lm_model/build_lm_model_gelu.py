...
def build_model(max_len=256,
    ...
    conv1 = Convolution1D(2 * embedding_dim, 24, padding='same')
    conv2 = Convolution1D(3 * embedding_dim, 16, padding='same')
    ...
    x = gelu1(x)
    x = dropout1(x, training = is_training)

    x = Flatten()(x)

    x = Dense(hidden_dim * 4)(x)
    x = gelu1(x)
    x = dropout2(x, training = is_training)
