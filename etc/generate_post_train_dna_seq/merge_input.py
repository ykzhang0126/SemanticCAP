...
if __name__ == '__main__':
    ...
    samples_num = load_data(root_path + '/data/post_train/input' + sys.argv[1] + '/train_valid_num.npy')
    X,Y = merge_input100(root_path + '/data/post_train/input' + sys.argv[1], 'train', samples_num[0: len(samples_num) - 1])
    print("total: %d samples for train, " % Y.shape[0], end = '')
    X,Y = merge_input100(root_path + '/data/post_train/input' + sys.argv[1], 'valid', samples_num[len(samples_num) - 1: len(samples_num)])
#   print(X[0].shape[0])
