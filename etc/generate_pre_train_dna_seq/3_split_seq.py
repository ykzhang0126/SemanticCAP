...
if __name__ == '__main__':
    ...
    seqs = load_data(root_path + '/data/pre_train/dna_train_source.npy')
    for index in range(0, len(seqs), batchNum): 
        ...
        np.save(root_path + '/data/pre_train/input' + str(max_len) + '/train_x_' + str(index // batchNum), x)
        np.save(root_path + '/data/pre_train/input' + str(max_len) + '/train_y_' + str(index // batchNum), y)
    ...
    # print("----------------validation----------------------")
    seqs = load_data(root_path + '/data/pre_train/dna_valid_source.npy')
    x, y = split_and_pad('bilstm', seqs, max_len, 2)
    np.save(root_path + '/data/pre_train/input' + str(max_len) + '/valid_x_0', x)
    np.save(root_path + '/data/pre_train/input' + str(max_len) + '/valid_y_0', y)
    tprint('%-8dsplitted context samples saved to %s' %
        (len(y), 'valid_0'))
    samples_num.append(len(y))
    # print(X[0].shape)
    ...
    np.save(root_path + '/data/pre_train/input' + str(max_len) + '/train_valid_num', samples_num)
