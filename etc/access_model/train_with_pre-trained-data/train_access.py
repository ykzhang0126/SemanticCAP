...
def train():
    ...
    model = None
    if method == 'flatten':
        model = build_access_model_flatten.build_model(is_training=True)
    if method == 'lstm':
        model = build_access_model_lstm.build_model(is_training=True)
    if method == 'sep':
        model = build_access_model_sep.build_model(is_training=True)
    ...
    opt = ta.optimizers.Lookahead(ta.optimizers.RectifiedAdam(learning_rate = 2e-5))
    ...
    history_callback = model.fit_generator(
        ...
        epochs=max_epoch,
        verbose=1,
        callbacks=[model_save, reduceLR, earlystopping],
...
def test(method):
    test_xi = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_x_in.npy', mode='r')
    test_xi = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_x_in.npy', mode='r',
                         shape=(test_xi.shape[0] // seq_len, seq_len))
    test_y = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_y.npy', mode='r')
    test_xf = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_xf.npy',
                         dtype='float32', mode='r', shape=(test_xi.shape[0], seq_len, feature_len))
    ...
    out_y = model.predict_generator(generator=datag, steps=test_y.shape[0] // batch_size).reshape(-1)
    pred_y = [1 if i > 0.5 else 0 for i in out_y]
    ...
    auroc = metrics.roc_auc_score(test_y, out_y)
    auprc = metrics.average_precision_score(test_y, out_y)
    f1 = metrics.f1_score(test_y, pred_y)
    print(f1, auroc, auprc)

def generate_feature_vec(method):
    test_xi = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_x_in.npy', mode='r')
    test_xi = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_x_in.npy', mode='r',
                         shape=(test_xi.shape[0] // seq_len, seq_len))
    test_y = np.memmap(root_path + '/data/' + tmode + '/input' + str(max_len) + '/valid_y.npy', mode='r')
    ...
    datag = Data_generator(DS_xf=test_xf, DS_x=test_xi, DS_y=test_y, batchSize=batch_size)
    submodel = Model(inputs = model.input, outputs = model.get_layer(index = 10).output)
    submodel.summary()
    out_y = submodel.predict_generator(generator=datag, steps=test_y.shape[0] // batch_size).reshape(-1, 16)
    # out_y = submodel.predict_generator(generator=datag, steps=10)
    test_y = test_y[0: test_y.shape[0] // batch_size * batch_size]
    # test_y = test_y[0: 320]
    print(out_y.shape)
    print(test_y.shape)
    with open('vec.txt', 'w') as fout:
        for i in range(len(test_y)):
            for x in out_y[i]:
                fout.write(str(float(x)))
                fout.write(' ')
            fout.write(str(int(test_y[i])))
            fout.write('\n')
...
if __name__ == '__main__':
    ...
    # train()
    # test('checkpoint')
    generate_feature_vec('checkpoint')
